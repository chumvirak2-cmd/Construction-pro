...
export default function MEPGasSystem() {
  ...
  const loadItems = () => {
    ...
    const gasItems = allItems.filter(item => item.category === 'Gas System')
    setItems(gasItems)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Gas System</h1>
          <p className="text-gray-500">Gas piping, detection, and safety systems</p>
        </div>
        <div className="flex gap-2">
          <input
            type="text"
            placeholder="Search gas system items..."
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredItems.length > 0 ? (
          ...
          <div className="col-span-full bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">No Gas System Items Yet</h3>
            <p className="text-gray-500">Start adding gas system components to your inventory.</p>
          </div>
        )}
      </div>
    </div>
